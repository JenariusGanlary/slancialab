...
const strategies = [
  {
    ...
    nicheTags: ["Indie SaaS", "Content Creator"],
...
async function main() {
  for (const strategy of strategies) {
    const existing = await prisma.strategy.findFirst({ where: { title: strategy.title } });
    if (existing) {
      await prisma.strategy.update({ where: { id: existing.id }, data: strategy });
    } else {
